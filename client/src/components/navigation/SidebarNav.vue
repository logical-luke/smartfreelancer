...
<template>
  <Transition name="fade" mode="in-out">
    <div v-if="isAuthorized" class="relative z-50">
      <nav class="lg:hidden py-6 px-4 bg-gray-800 dark:bg-gray-900">
        <div class="flex flex-row items-center justify-between mr-2">
          ...
          <div class="flex-row flex items-center justify-end gap-2">
            <button
                class="navbar-burger p-4 bg-indigo-500 dark:bg-indigo-600 text-white flex items-center rounded focus:outline-none"
                @click="toggle"
            >
      ...
      <div :class="{ hidden: !open }" class="lg:block navbar-menu relative z-50">
        <div class="navbar-backdrop fixed lg:hidden inset-0 bg-gray-800 dark:bg-gray-900 opacity-10"></div>
        <nav
            class="flex fixed top-0 left-0 bottom-0 flex-col w-full lg:max-w-xs lg:w-3/4 pt-6 pb-8 bg-gray-800 dark:bg-gray-900 overflow-y-auto">
          <div class="flex flex-row justify-between items-center px-4 pb-8 mr-2 lg:mr-0 mb-8">
            ...
            <div class="flex flex-row items-center justify-end gap-2">
              <button
                  class="lg:hidden navbar-burger p-4 bg-indigo-500 dark:bg-indigo-600 text-white flex items-center rounded focus:outline-none"
                  @click="toggle"
              >
                <i class="text-xl pi pi-bars"></i>
              </button>
