<template>
  ...
  >
    <div :class="this.isRunning ? 'motion-safe:animate-pulse' : ''">
      <img
        v-if="this.isMainIconPomodoro()"
        src="/tomato-icon.svg"
        alt="Pomodoro Timer Tomato"
      />
  ...
  <overlay-panel ref="op" :show-close-icon="false">
    <div class="w-40">
      ...
      <div
        v-if="this.configuration.settingsExpanded"
        class="flex-col mt-4 gap-2 items-center justify-center"
      >
        <div class="text-center mx-auto flex-row items-center justify-center">
          <button
            ...
            class="inline-flex mx-2 bg-indigo-500 hover:bg-indigo-600 items-center justify-center px-2 py-2 text-sm font-medium text-white rounded-full transition duration-200"
          >
        ...
        <div class="flex-auto my-2">
          <label for="work-duration" class="font-bold block mb-2">
            {{ $t("Work duration") }}:
          </label>
          <input-number
            :min="1"
          ...
          <div class="flex mt-2 space-x-4">
            <p
              v-for="n in [20, 25, 30, 55]"
              :key="n"
              :class="{ 'font-bold': this.configuration.workDuration === n }"
              @click="this.setWorkDuration(n)"
            >
              {{ n }}
            </p>
          </div>
        ...
        <div class="flex-auto my-2">
          <label for="break-duration" class="font-bold block mb-2">
            {{ $t("Break") }}:
          </label>
          <input-number
            :min="0"
        ...
        <div class="flex-auto my-2">
          <label for="repeat" class="font-bold block mb-2">
            {{ $t("Repeat") }}:
          </label>
          <input-number
            :min="0"
...
export default {
  ...
  computed: {
    ...
    pomodoroIconConditionalClasses() {
      return (
        this.sizeClasses +
        " " +
        (this.isPomodoroTypeSetInSettings() ? "bg-indigo-600" : "")
      );
    },
  ...
  methods: {
    ...
    async toggleSettingsExpanded() {
      this.configuration.settingsExpanded =
        !this.configuration.settingsExpanded;
    ...
    isMainIconPomodoro() {
      return (
        (this.pomodoro && this.pomodoro.type === "pomodoro") ||
        (!this.pomodoro && this.configuration.type === "pomodoro")
      );
    },
    isMainIconBreak() {
