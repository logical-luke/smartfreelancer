  ...
  <tr :class="greyBackground ? 'bg-gray-50' : ''" class="text-xs">
    ...
    <td class="p-2">
      <div class="flex items-center space-x-2">
        <toggle-timer-button :size="8" :task-id="id" />
        <edit-button class="mt-1" :goTo="`/task/edit/${id}`" :icon-only="true" />
        <delete-button @delete="deleteTask(id)" :subject="name" :icon-only="true" />
      </div>
    </td>
  </tr>
</template>
